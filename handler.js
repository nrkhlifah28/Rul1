...
module.exports = {
    ...
    async participantsUpdate({ id, participants, action }) {
        ...
        switch (action) {
                       ...
                       let pp = 'https://telegra.ph/file/58acbba00bf7570860e61.png'
                        try {
                            ...
                            text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Yah,si Beban Masuk Grup @user').replace('@subject', groupMetadata.subject).replace('@desc', groupMetadata.desc.toString()) :
                                ...
                                this.send2ButtonImg(id, pp, text, "NoerBotz", "Script Bot", ".sc", "Tampilan Menu", ".menu", fake, { contextInfo: { externalAdReply :{
            ...
            case 'promote':
                text = (chat.sPromote || this.spromote || conn.spromote || '@user ```Kamu sekarang Admin```')
            case 'demote':
                if (!text) text = (chat.sDemote || this.sdemote || conn.sdemote || '@user ```Lo bukan  Admin lagi```')
...
`.trim(), msg, {
 ...
 global.dfail = async (type, m, conn) => {
    let msg = {
        ...
        group: 'Perintah ini hanya dapat digunakan di grup!\\n*Gc Bot © NoerBoTZ-Mdོ :* https://chat.whatsapp.com/FM4JHZbUJZi0m786dBZrsr',
...
global.thumb = 'https://telegra.ph/file/0b6a1d55e82fa0fcb801e.jpg'
